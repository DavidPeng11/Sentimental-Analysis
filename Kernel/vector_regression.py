...
if __name__ == '__main__':
    ...
    train_list, test_list= train_test_split(match_list,test_size=0.4,random_state=0)
    x_train,y_train=MakeArray(train_list, seed_word_valence, model)
    print ("x_train's shape is %d "% x_train.shape)
    print ("y_train's shape is %d "% y_train.shape)

    x_test,y_test=MakeArray(test_list, seed_word_valence,model)
    print ("x_test's shape is %d " % x_test.shape)
    print ("y_test's shape is %d " % y_test.shape)
